...
from updater import api, get_config, main
from updater.config import create_example_env, print_config, validate_config
...
Examples:
  ...
  %(prog)s --create-env             # Create .env file from example
    ...
    parser.add_argument(
        "--mod-id", type=str, help="Override the mod ID from environment"
    )

    parser.add_argument(
        "--download-path", type=Path, help="Override the download path from environment"
    )
    ...
    parser.add_argument(
        "--validate-key", action="store_true", help="Validate API key and exit"
    )
    ...
    parser.add_argument(
        "--version", action="version", version="CurseForge Auto-Updater 1.0.0"
    )
